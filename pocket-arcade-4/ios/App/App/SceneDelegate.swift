...
class SceneDelegate: UIResponder, UIWindowSceneDelegate {
    ...
    @objc private func resetTapped() {
        ...
        let alert = UIAlertController(
            title: "Reset Code?",
            message: "Restores your last saved backup if you made one, otherwise the app's original files. This can't be undone.",
            preferredStyle: .alert
        )
        alert.addAction(UIAlertAction(title: "Cancel", style: .cancel))
        alert.addAction(UIAlertAction(title: "Reset", style: .destructive) { _ in
            EditableAssets.resetToBackupOrBundle()
...
enum EditableAssets {
    ...
    static var backupURL: URL { documentsURL.appendingPathComponent("www-backup", isDirectory: true) }
    ...
    // separate untouched reference copy (www-factory) used for "restore
    // factory defaults."
    ...
    static func resetToBackupOrBundle() {
        let fm = FileManager.default
        let source = fm.fileExists(atPath: backupURL.path) ? backupURL : factoryURL
        try? fm.removeItem(at: wwwURL)
        if fm.fileExists(atPath: source.path) {
            try? fm.copyItem(at: source, to: wwwURL)
        } else {
            copyBundleTo(wwwURL)
        }
    }
