...
**Backups, from Settings:** tap "Save Backup Now" before making risky edits.
If something breaks, tap "Restore Backup" to undo back to that point, or
"Restore Factory Defaults" to wipe all edits and go back to the original
app. Both work from inside the app, backed by Capacitor's official
Filesystem plugin.
...
your last saved backup if you made one, otherwise the original app files.

## Honesty about the native (Swift) parts
The Documents-editing, backup, and reset-button features required a small
amount of hand-written Swift in `ios/App/App/SceneDelegate.swift`. That
code has been written carefully and checked for syntax, but this build
environment has no way to actually compile and run it — so unlike the rest
of this app, it hasn't been verified end-to-end on a real device. If
something in this specific area misbehaves after building, tell me exactly
what happened and I'll fix it precisely rather than guessing again.
