...
window.Games.stardodger = (function () {
  ...
  function update(dt) {
    ...
    for (let i = rocks.length - 1; i >= 0; i--) {
      ...
      if (!r.grazed && dist < r.r + ship.r + 26 && dist > r.r + ship.r) {
        r.grazed = true; score += 25; addBurst(ship.x, ship.y, '#7ef9ff', 6);
      }
  ...
  function drawShip() {
    ...
    grad.addColorStop(0, 'rgba(126,249,255,0.9)'); grad.addColorStop(1, 'rgba(126,249,255,0)');
    ...
    ctx.closePath(); ctx.fillStyle = '#eaf2ff'; ctx.fill();
    ctx.strokeStyle = '#4bd0ff'; ctx.lineWidth = 2; ctx.stroke();
    ctx.restore();
  }
  ...
  function drawRock(r) {
    ...
    ctx.closePath(); ctx.fillStyle = '#3a3f5c'; ctx.fill();
    ctx.strokeStyle = '#8a93c4'; ctx.lineWidth = 1.5; ctx.stroke();
    ctx.restore();
  }
  ...
  function draw() {
    ...
    bg.addColorStop(0, '#0a0c1e'); bg.addColorStop(1, '#05060f');
